...
$(document).ready(function () {
  //jab browser pura ka pura load hojaye toh uske bad iske andar ka code chale.
  ...
  for (i = 1; i < 100; i++) {
    let row = $(`<div class="cell-row"></div>l`);
    for (j = 1; j < 100; j++) {
  ...
  $(".input-cell").click(function (e) {
    if (e.ctrlKey) {
      ...
      if (rowId > 1) {
        let topCellSelected = $(`#row-${rowId - 1}-colId-${colId}`).hasClass(
          "selected"
        );
        if (topCellSelected) {
          $(this).addClass("top-cell-selected");
          $(`#row-${rowId - 1}-colId-${colId}`).addClass(
      ...
      if (rowId < 100) {
        let bottomCellSelected = $(`#row-${rowId + 1}-colId-${colId}`).hasClass(
          "selected"
        );
        if (bottomCellSelected) {
          $(this).addClass("bottom-cell-selected");
          $(`#row-${rowId + 1}-colId-${colId}`).addClass("top-cell-selected");
        }
      }

      if (colId > 1) {
        let leftCellSelected = $(`#row-${rowId}-colId-${colId - 1}`).hasClass(
          "selected"
        );
        if (leftCellSelected) {
          $(this).addClass("left-cell-selected");
          $(`#row-${rowId}-colId-${colId - 1}`).addClass("right-cell-selected");
        }
      }

      if (colId < 100) {
        let rightCellSelected = $(`#row-${rowId}-colId-${colId + 1}`).hasClass(
          "selected"
        );
        if (rightCellSelected) {
          $(this).addClass("right-cell-selected");
          $(`#row-${rowId}-colId-${colId + 1}`).addClass("left-cell-selected");
        }
      }
    } else {
  ...
  function changeHeader(ele) {
    ...
    $(".text-color-picker").val(cellInfo["background-color"]);
  ...
  $(".input-cell.selected").each(function () {
    ...
    if (cellData[selectedSheet][rowId]) {
      if (cellData[selectedSheet][rowId][colId]) {
        ...
        cellData[selectedSheet][rowId][colId] = value;
      }
    } else {
      cellData[selectedSheet][rowId] = {};
      cellData[selectedSheet][rowId][colId] = { ...defaultProperties };
...
function emptySheet() {
  ...
  for (let i of Object.keys(sheetInfo)) {
    for (let j of Object.keys(sheetInfo[i])) {
      ...
      $(`#row-${i}-col-${j}`).css("font-weight", "Arial");
      $(`#row-${i}-col-${j}`).css("font-style", "Arial");
